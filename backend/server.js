...
// ✅ Parent Login API (Fetches Only Student Data)
// ✅ Parent Login API (Fetches Student Details)
app.post("/parent-login", (req, res) => {
  const { email, password } = req.body;
  console.log("Parent Login Data:", req.body);
  ...
  db.get("SELECT * FROM parents WHERE email = ?", [email], (err, parent) => {
    ...
    console.log("Parent Logged In:", parent);

    // Fetch student details using student_id from the parent record
    db.get("SELECT * FROM users WHERE hallticketnumber = ?", [parent.student_id], (err, student) => {
      if (err) return res.status(500).json({ message: "Error fetching student data." });
      if (!student) return res.status(404).json({ message: "Student data not found." });

      console.log("Student Data for Parent:", student);
